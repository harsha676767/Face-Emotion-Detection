...
def Train(epochs, train_loader, val_loader, criterion, optimizer, device):
    ...
    for e in range(epochs):
        ...
        train_loss = train_loss / len(train_loader)  # Change to use len(train_loader)
        train_acc = train_correct.double() / len(train_loader.dataset)  # len(train_loader.dataset) gives the number of samples
        validation_loss = validation_loss / len(val_loader)  # Change to use len(val_loader)
        val_acc = val_correct.double() / len(val_loader.dataset)  # len(val_loader.dataset) gives the number of samples
...
if __name__ == '__main__':
    ...
    parser.add_argument('-hparams', '--hyperparams', type=bool, help='True when changing the hyperparameters')
    ...
    if args.train:
        ...
        test_img_dir = os.path.join(args.data, 'test')

        transformation = transforms.Compose([transforms.ToTensor(), transforms.Normalize((0.5,), (0.5,))])

        # Dataset loading
        train_dataset = Plain_Dataset(data_dir=train_img_dir, transform=transformation)
        test_dataset = Plain_Dataset(data_dir=test_img_dir, transform=transformation)
        ...
        # Start training
        Train(epochs, train_loader, test_loader, criterion, optimizer, device)
